# PREREQUISTE : 
# 1. All the zip and jar files should exists in the modules folder of EE

# BELOW STEPS WILL
# 1. Copy the zip file PushArtificatsToTeamBucket-Lambda.zip to the modules folder of EE 
# 2. Deploy the lambda 
# 3. Invoke the lamda that will create the bucket and will trigger copy of files from EE module to a bucket in team account


export AWS_ACCESS_KEY_ID=ASIA265Q4PFVOKLAWU4Q
export AWS_SECRET_ACCESS_KEY=2MCgldXz9VLsre/jE0hbyf0+rMSEeEpl1z/hA0v4
export AWS_SESSION_TOKEN=
export AWS_DEFAULT_REGION=us-east-1

zip -r PushArtificatsToTeamBucket-Lambda.zip PushArtificatsToTeamBucket-Lambda.py
aws s3 cp PushArtificatsToTeamBucket-Lambda.zip s3://ee-assets-prod-us-east-1/modules/f10ef3b4f001409c893ee7f9decab4c8/v1/PushArtificatsToTeamBucket-Lambda.zip
aws cloudformation deploy --template-file PublishArtifactsToTeamBucket.yaml  --stack-name eventengine --capabilities=CAPABILITY_IAM